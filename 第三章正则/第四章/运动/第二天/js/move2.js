(function () {
    ...
    /**
     * move方法:用来封装运动元素
     * @param opt:{id:元素,target:要运动的目标位置,duration:总时间,effect:运动效果,callback:运动完的回调函数}
     *
     * */
    function move(opt) {//私有作用域中的一个私有函数
            //1.为Linear公式的四个参数做准备
        if(!opt.id)return;//当没有传这个id直接阻断程序执行;
        var tmpEffect=null;
        ...
        for(var attr in opt){
            defaultOpt[attr]=opt[attr];//用户传了,按用户传了的来,否则默认值来
        }
        var curEle=defaultOpt.id;
        var target=defaultOpt.target;
        clearInterval(curEle.timer);
        if(typeof  defaultOpt.effect==='number'){
            var str=ary[defaultOpt.effect%ary.length];//%思想的运用:有几种情况就%几
            ary=str.split('-');
            tmpEffect=ary.length>=2?animationEffect[ary[0]][ary[1]]:animationEffect[ary[0]];
        }else if(typeof defaultOpt.effect==='object'){
            tmpEffect=defaultOpt.effect.length>=2?animationEffect[defaultOpt.effect[0]][defaultOpt.effect[1]]:animationEffect[defaultOpt.effect[0]];
        }
        var begin={},change={};
        var time=0;
        ...
        //2.开启定时器,根据Linear公式求出最新位置,并且设置最新位置
        curEle.timer=setInterval(function () {
            //2.1让时间不断累加
              ...
              for(var attr in change){
                  var curPos=tmpEffect(time,begin[attr],change[attr],defaultOpt.duration);
